...
class SmartLock:
    ...
    def socketInit(self, proxyPort):
        ...
        self.socket = context.socket(
            zmq.PUB)  # criação de um socket do tipo PUB (pois ele será quem publica as atualizações)
        self.socket.connect(
            "tcp://localhost:%d" % proxyPort)  # conecta este socket ao proxy (ver classe Proxy em ./proxy.py)
